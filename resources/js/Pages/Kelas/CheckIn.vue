<template>
  <AppLayout title="Check-in Kelas">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <PageHeader
        title="Check-in Kelas"
        subtitle="Catat kehadiran member bulanan dan harian untuk kelas"
      >
        <template #actions>
          <Button
            variant="secondary"
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div>
              ...
              >
                ...
                <option v-for="kelas in kelasList" :key="kelas.id" :value="kelas.id">
                  {{ kelas.name }} 
                  <span v-if="kelas.monthly_price > 0 || kelas.daily_price > 0">
                    (Monthly: {{ formatCurrency(kelas.monthly_price) }}, Daily: {{ formatCurrency(kelas.daily_price) }})
                  </span>
                </option>
              </select>
              ...
              <div class="flex gap-4">
                <label class="flex items-center cursor-pointer">
                  <input
                    ...
                    @change="resetForm"
                  />
                  <span class="ml-2 text-sm font-medium text-gray-700">Member Bulanan</span>
                </label>
                <label class="flex items-center cursor-pointer">
                  <input
                    v-model="form.type"
                    ...
                    @change="resetForm"
                  />
                  <span class="ml-2 text-sm font-medium text-gray-700">Member Harian</span>
                </label>
              </div>
            </div>
            ...
            <div v-if="form.type === 'monthly'">
              <label for="member_search" class="block text-sm font-medium text-gray-700 mb-2">
                Cari Member Bulanan <span class="text-red-500">*</span>
              </label>
              <input
                id="member_search"
            ...
            <div v-if="form.type === 'daily'" class="space-y-4">
              ...
              <div v-if="selectedKelas" class="p-3 bg-blue-50 rounded-lg text-sm">
                <div class="font-medium text-gray-900">Harga Check-in Harian:</div>
                <div class="text-blue-700 font-semibold">{{ formatCurrency(selectedKelas.daily_price) }}</div>
                <p class="text-xs text-gray-600 mt-1">Harga akan ditagihkan saat check-in</p>
              </div>
            </div>

            <!-- Note for monthly members -->
            <div v-if="form.type === 'monthly' && selectedKelas" class="p-3 bg-green-50 rounded-lg text-sm">
              <div class="font-medium text-gray-900">Member Bulanan</div>
              <p class="text-xs text-gray-600">Member bulanan tidak dikenakan biaya check-in karena sudah membayar saat registrasi</p>
            </div>
            ...
            <Button
              type="submit"
              variant="primary"
              :disabled="form.processing || !form.kelas_id"
              class="w-full"
            >
                ...
                <div class="flex-1">
                  ...
                  <div class="flex gap-2 mt-2">
                    <span :class="[
                      ...
                      attendance.type === 'monthly' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                    ]">
                      {{ attendance.type === 'monthly' ? 'Monthly' : 'Daily' }}
                    </span>
                    <span class="inline-block px-2 py-0.5 text-xs font-semibold text-green-800 bg-green-100 rounded">
                      {{ formatCurrency(attendance.amount) }}
                    </span>
                  </div>
                </div>
                <Button
...
const selectedKelas = computed(() => {
  ...
  return props.kelasList?.find(k => k.id === parseInt(form.kelas_id))
})
...
function searchMembers() {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      const response = await axios.get('/kelas/checkin/search-members', {
...
function checkout(attendanceId) {
  router.put(`/kelas/checkin/${attendanceId}/checkout`, {}, {
    preserveScroll: true
  })
}
</script>
